                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 5 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                TriggerEvent('qb-armoralerts:ToggleDuty', onDuty)
                            end
                ...
                for k, v in pairs(Config.Locations["evidence"]) do
                    if #(pos - vector3(v.x, v.y, v.z)) < 2 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.0 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "armorevidence", {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", "armorevidence")
                            end
                ...
                for k, v in pairs(Config.Locations["evidence2"]) do
                    if #(pos - vector3(v.x, v.y, v.z)) < 2 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.0 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - evidence stash")
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "armorevidence2", {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", "armorevidence2")
                            end
                        elseif #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "evidence stash")
                        end
                ...
                for k, v in pairs(Config.Locations["evidence3"]) do
                    if #(pos - vector3(v.x, v.y, v.z)) < 2 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.0 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Evidence stash")
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "armorevidence3", {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", "armorevidence3")
                            end
                        elseif #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "evidence stash")
                        end
                ...
                for k, v in pairs(Config.Locations["trash"]) do
                    if #(pos - vector3(v.x, v.y, v.z)) < 2 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.0 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "armortrash", {
                                    maxweight = 4000000,
                                    slots = 300,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", "armortrash")
                            end
                        elseif #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 4.5 and IsArmoryWhitelist() then
                        if onDuty then
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    SetWeaponSeries()
                                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "armor", Config.Items)
                                end
                            elseif #(pos - vector3(v.x, v.y, v.z)) < 2.5 then
                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 4.5 then
                        if onDuty then
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "armorstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                    TriggerEvent("inventory:client:SetCurrentStash", "armorstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                end
                            elseif #(pos - vector3(v.x, v.y, v.z)) < 2.5 then
                            ...
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    ...
                                    if player ~= -1 and distance < 2.5 then
                                        ...
                                        TriggerServerEvent("armor:server:showFingerprint", playerId)
                                    else
...
RegisterNetEvent('armor:client:showFingerprint')
AddEventHandler('armor:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('armor:client:showFingerprintId')
AddEventHandler('armor:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(data)
    TriggerServerEvent('armor:server:showFingerprintId', FingerPrintSessionId)
end)
...
RegisterNetEvent('armor:client:SendEmergencyMessage')
AddEventHandler('armor:client:SendEmergencyMessage', function(message)
    local coords = GetEntityCoords(PlayerPedId())
    
    TriggerServerEvent("armor:server:SendEmergencyMessage", coords, message)
    TriggerEvent("armor:client:CallAnim")
end)

RegisterNetEvent('armor:client:EmergencySound')
AddEventHandler('armor:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('armor:client:CallAnim')
AddEventHandler('armor:client:CallAnim', function()
...
RegisterNetEvent('armor:client:ImpoundVehicle')
AddEventHandler('armor:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle ~= nil then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("armor:server:Impound", plate, fullImpound, price)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('armor:client:CheckStatus')
AddEventHandler('armor:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "armor" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('armor:GetPlayerStatus', function(result)
                    if result ~= nil then
...
function ImpoundVehicleList()
    QBCore.Functions.TriggerCallback("armor:GetImpoundedVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Vehicles:"
        ClearMenu()
...
function TakeOutImpound(vehicle)
    ...
    QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
        QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
            ...
            TriggerServerEvent('armor:server:TakeOutImpound',vehicle.plate)
            closeMenuFull()
            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
            SetVehicleEngineOn(veh, true, true)
...
function IsArmoryWhitelist()
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'armor' then
        retval = true
    end
