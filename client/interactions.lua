...
RegisterNetEvent('armor:client:SetOutVehicle')
AddEventHandler('armor:client:SetOutVehicle', function()
...
RegisterNetEvent('armor:client:PutInVehicle')
AddEventHandler('armor:client:PutInVehicle', function()
...
RegisterNetEvent('armor:client:SearchPlayer')
AddEventHandler('armor:client:SearchPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", playerId)
        TriggerServerEvent("armor:server:SearchPlayer", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('armor:client:SeizeCash')
AddEventHandler('armor:client:SeizeCash', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("armor:server:SeizeCash", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('armor:client:SeizeDriverLicense')
AddEventHandler('armor:client:SeizeDriverLicense', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("armor:server:SeizeDriverLicense", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)
...
RegisterNetEvent('armor:client:RobPlayer')
AddEventHandler('armor:client:RobPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
...
RegisterNetEvent('armor:client:JailCommand')
AddEventHandler('armor:client:JailCommand', function(playerId, time)
    TriggerServerEvent("armor:server:JailPlayer", playerId, tonumber(time))
end)
...
RegisterNetEvent('armor:client:JailPlayer')
AddEventHandler('armor:client:JailPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
...
RegisterNetEvent('armor:client:BillPlayer')
AddEventHandler('armor:client:BillPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        DisplayOnscreenKeyboard(1, "", "", "", "", "", "", 20)
...
RegisterNetEvent('armor:client:PutPlayerInVehicle')
AddEventHandler('armor:client:PutPlayerInVehicle', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("armor:server:PutPlayerInVehicle", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('armor:client:SetPlayerOutVehicle')
AddEventHandler('armor:client:SetPlayerOutVehicle', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("armor:server:SetPlayerOutVehicle", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('armor:client:EscortPlayer')
AddEventHandler('armor:client:EscortPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("armor:server:EscortPlayer", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('armor:client:KidnapPlayer')
AddEventHandler('armor:client:KidnapPlayer', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent("armor:server:KidnapPlayer", playerId)
            end
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
...
RegisterNetEvent('armor:client:CuffPlayerSoft')
AddEventHandler('armor:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                TriggerServerEvent("armor:server:CuffPlayer", playerId, true)
                HandCuffAnimation()
            else
...
RegisterNetEvent('armor:client:CuffPlayer')
AddEventHandler('armor:client:CuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    local playerId = GetPlayerServerId(player)
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                        TriggerServerEvent("armor:server:CuffPlayer", playerId, false)
                        HandCuffAnimation()
                    else
...
RegisterNetEvent('armor:client:GetEscorted')
AddEventHandler('armor:client:GetEscorted', function(playerId)
...
RegisterNetEvent('armor:client:DeEscort')
AddEventHandler('armor:client:DeEscort', function()
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(PlayerPedId(), true, false)
end)

RegisterNetEvent('armor:client:GetKidnappedTarget')
AddEventHandler('armor:client:GetKidnappedTarget', function(playerId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('armor:client:GetKidnappedDragger')
AddEventHandler('armor:client:GetKidnappedDragger', function(playerId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('armor:client:GetCuffed')
AddEventHandler('armor:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent("armor:server:SetHandcuffStatus", true)
        ClearPedTasksImmediately(PlayerPedId())
...
function IsTargetDead(playerId)
    local retval = false
    QBCore.Functions.TriggerCallback('armor:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
